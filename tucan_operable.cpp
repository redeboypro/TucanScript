...
namespace tucan_script 
{
	...
	{
		set(rValue.getType(), rValue.m_stringValue, rValue.m_intValue, rValue.m_floatValue, rValue.m_boolValue, rValue.m_arrayValues);
	}
	...
	tucan_operable& tucan_operable::operator[](const size_t& index)
	{
	...
	{
		if (!isArray())
			set(TUCAN_TYPE::ARRAY, std::string(), 0, 0.0, false, std::vector<tucan_operable>());

		size_t fixedSize = length();

		if (index >= fixedSize)
			for (size_t i = 0; i < fixedSize - index + 1; i++)
				m_arrayValues.push_back(tucan_operable(TUCAN_TYPE::INTEGER));
